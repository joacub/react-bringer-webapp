...
Twitter.prototype.__buildEndpoint = function (path, base) {
  ...
  const isFullUrl = (url.parse(path).protocol !== null);
  if (isFullUrl) {
    endpoint = path;
  } else {
...
Twitter.prototype.__request = function (method, path, params, callback) {
  ...
  if (promise) {
    ...
    return new Promise(((resolve, reject) => {
      _this.request(options, (error, response, data) => {
        // request error
        if (error) {
          return reject(error);
        }
...
Twitter.prototype.stream = function (method, params, callback) {
  ...
  const url = this.__buildEndpoint(method, base);
  const request = this.request({ url, qs: params });
